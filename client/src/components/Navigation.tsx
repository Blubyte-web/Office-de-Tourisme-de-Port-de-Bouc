            ...
            <div className="px-4 py-4 space-y-3">
              {navItems.map((item) => (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault(); // evita comportamento di default del link
                    setIsOpen(false); // chiude il menu hamburger
                    const target = document.querySelector(item.href); // trova la sezione
                    if (target) {
                      setTimeout(() => {
                        target.scrollIntoView({ behavior: "smooth" }); // scroll fluido
                      }, 300); // attende un attimo dopo la chiusura del menu
                    }
                  }}
